...
def upload():
    ...
    try:
        ...
        yaml_match_count = 0
        dtype_map = {
            'runs_total': 'int8', 'runs_batter': 'int8', 'runs_extras': 'int8',
            'wides': 'int8', 'noballs': 'int8', 'byes': 'int8', 'legbyes': 'int8',
            'is_legal_ball': 'int8', 'is_boundary': 'int8', 'is_dot_ball': 'int8',
            'wicket_flag': 'int8', 'over': 'int8', 'ball_in_over': 'int8',
            'innings_number': 'int8',
        }
        if filename.endswith('.csv'):
            chunks = []
            for chunk in pd.read_csv(file, chunksize=10000, dtype=dtype_map, on_bad_lines='skip'):
                chunks.append(chunk)
            ...
            if df is not None and len(df) == 0:
                return jsonify({'success': False, 'error': 'No matches found matching your filters. Try broadening your team or year range.'})
            if df is not None:
