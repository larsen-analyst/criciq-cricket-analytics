...
def convert_yaml_to_df(file_content, filename, team_filter=None, year_from=None, year_to=None):
                    ...
                    try:
                        ...
                        if err:
                            ...
                            if match_count % BATCH_SIZE == 0:
                                batch_dfs.append(pd.DataFrame(all_rows))
                                all_rows = []
    ...
    if not all_rows and not batch_dfs:
    ...
    df = pd.concat(batch_dfs, ignore_index=True) if len(batch_dfs) > 1 else batch_dfs[0]
    del batch_dfs

    int_cols = {
        'innings_number': 'int8', 'over': 'int8', 'ball_in_over': 'int8',
